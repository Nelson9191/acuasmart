...
int am_mqtt_start( bool awsIotMqttMode,
                 ...
                 const IotNetworkInterface_t * pNetworkInterface )
{
    ...
    const char * pTopics[ 2 ] =
    {
        MQTT_PUBLISH_INPUT_TOPIC,
        MQTT_PUBLISH_HEARTBEAT_TOPIC,
    };
    ...
    if( status == EXIT_SUCCESS )
    {
        ...
        status = _am_mqtt_establish_connection( awsIotMqttMode,
                                                pIdentifier,
                                                pNetworkServerInfo,
    ...
    if( status == EXIT_SUCCESS )
    {
        ...
        status = _am_mqtt_modify_subscriptions( mqttConnection,
                                       IOT_MQTT_SUBSCRIBE,
                                       pTopics,
                                       &publishesReceived );
    }
    ...
    if( status == EXIT_SUCCESS )
    {
                                 ...
                                 IOT_DEMO_MQTT_PUBLISH_BURST_SIZE ) == true )
        {
            ...
            status = _am_mqtt_publish_all_messages( mqttConnection,
                                          pTopics,
                                          &publishesReceived );
                                 ...
                                 void * pCallbackParameter )
{
    ...
    for( i = 0; i < TOPIC_FILTER_COUNT; i++ )
    {
        ...
        pSubscriptions[ i ].callback.function = _am_mqtt_subscription_callback;
    }
                                ...
                                IotSemaphore_t * pPublishReceivedCounter )
{
    ...
    publishComplete.function = _am_mqtt_operation_complete_allback;
